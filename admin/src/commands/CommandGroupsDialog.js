...
export default function CommandGroupsDialog({ command, groups, allGroups }) {
    ...
    const handleSubmit = () => {
        fetch(`/adm/commandgroup/${command.id}?csrf=${window.csrf}`, {
            method: "POST",
            headers: {
            ...
            setShowDialog(false);
            if (resp.status !== 200) {
                alert("Could not set command groups!");
            }
        });
    };
